...
export default function TransactionsTable() {
  ...
  const [filter, setFilter] = useQueryState<TransactionsFilters>(
    "filter",
    parseAsStringEnum<TransactionsFilters>(Object.values(TransactionsFilters)),
  );
  ...
  const { result, isLoading, error } = useApiQuery({
    ...
    retry: 2,
    enabled:
      !!user && ["donor", "admin"].some((role) => user.roles.includes(role)),
  });
    ...
    <div className="flex flex-col gap-6 mt-6 w-full">
      ...
      <div className="w-auto flex flex-wrap justify-start gap-4 px-4">
        <div className="flex gap-2 items-center bg-primary rounded-xl shadow-md">
          {Object.values(TransactionsFilters).map((status) => (
            <div
              key={status}
